...
-select the IAM role that allows your Redshift cluster permission to access S3 bucket
-JSON path file tells Redshift how to parse your raw JSON files and map to columns in staging_events table.
-Region is 'us-west-2' where the bucket is located.
"""
staging_events_copy = ("""
COPY staging_events
FROM 's3://your-bucket/log_data'
IAM_ROLE 'arn:aws:iam::your-iam-role-id:role/RedshiftCopyRole'
JSON 's3://your-bucket/log_json_path.json'
REGION 'us-west-2';
""")

"""
-Pulling song metadata from S3 into staging_songs table.
-Since song data is structured well, you can use 'auto' keyword so Redshift auto-maps the fields to
columns in staging_songs table
"""
staging_songs_copy = ("""
COPY staging_songs
FROM 's3://your-bucket/song_data'
IAM_ROLE 'arn:aws:iam::your-iam-role-id:role/RedshiftCopyRole'
JSON 'auto'
REGION 'us-west-2';
""")
